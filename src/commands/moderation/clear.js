  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!messages || (typeof messages.size === 'number' ? messages.size === 0 : messages.length === 0)) {
        return interaction.editReply('❌ Nessun messaggio da cancellare.').catch(() => {});
      }
      const deleted = await interaction.channel.bulkDelete(messages, true);
      if (!deleted || deleted.size === 0) {
        return interaction.editReply('❌ Nessun messaggio cancellato (solo messaggi degli ultimi 14 giorni si possono eliminare in blocco).').catch(() => {});
      }
      await interaction.editReply(`🧹 Cancellati **${deleted.size}** messaggi${user ? ` di ${user.tag}` : ''}.`).catch(() => {});
    } catch (e) {
      console.error(e);
      if (e?.code === 50034 || e?.message?.includes('14')) {
        await interaction.editReply('❌ Impossibile cancellare (messaggi più vecchi di 14 giorni non si possono eliminare in blocco).').catch(() => {});
      } else {
        await interaction.editReply('❌ Impossibile cancellare i messaggi. Verifica i miei permessi (Gestisci Messaggi, Leggere Cronologia).').catch(() => {});
      }
    }
  },
};
